...
> This is the complete technical write-up. For the summary with figures, see [README.md](README.md).
...
The `-points.csv` file alone does not carry player names, but tour and player identity are recoverable: the `match_id` block distinguishes men's from women's singles, and `2024-usopen-matches.csv` supplies names. The statistics reported below therefore **pool both tours**, which is a limitation of this analysis rather than of the dataset. Splitting by tour is an open item.
...
> **Scope note.** This section is observational and sits outside the main computational contribution. Serve speed is confounded with player quality and with first-versus-second-serve composition, so the coefficients below describe association within this dataset only.
...
```text
00_usopen_data_audit.ipynb  # not yet committed; see Open Items
...
> **Under review.** This figure and `supporting/19_spin_azimuth_heatmap_alternate.png` disagree about whether
> admissible width varies primarily with spin or with azimuth, and neither reproduces the 1D results reported
> above. The map is being re-verified against the 1D boundary solver before it is restored. See Open Items.
