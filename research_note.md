# Research Note — The Serve Admissibility Envelope

## Full derivations, verification detail, and extended discussion

> Complete technical write-up. For the illustrated summary, see [README.md](README.md).
...
$$
m\dot{\mathbf v} = m\mathbf g - \frac12\rho A C_D \|\mathbf v\|\mathbf v + \frac12\rho A C_L \|\mathbf v\|^2 \left( \hat{\boldsymbol\omega} \times \hat{\mathbf v} \right)
$$
...
$$
\mathbf a_D = - \frac{1}{2m} \rho A C_D \|\mathbf v\|\mathbf v
$$
...
$$
\omega_{\mathrm{rad/s}} = \omega_{\mathrm{rpm}} \frac{2\pi}{60}.
$$
...
$$
\mathcal A = \{ \theta,\phi: \text{trajectory satisfies all legal constraints} \}.
$$
...
The `-points.csv` file alone does not carry player names, but tour and player identity are recoverable: the `match_id` block distinguishes men's from women's singles, and `2024-usopen-matches.csv` supplies names. The statistics below therefore **pool both tours**, which is a limitation of this analysis rather than of the dataset. Splitting by tour is an open item.
...
> **Scope note.** Observational and outside the main computational contribution. Serve speed is confounded with player quality and with first-vs-second-serve composition, so these coefficients describe association within this dataset only.
...
```text
00_usopen_data_audit.ipynb  # not yet committed
...
![Admissibility envelope](figures/main/01_admissibility_envelope.png)
...
> **Under review.** This figure and `supporting/19_spin_azimuth_heatmap_alternate.png` disagree about whether admissible width varies primarily with spin or with azimuth, and neither reproduces the 1D results reported above. The map is being re-verified against the 1D boundary solver before it is restored.
...
![Usopen speed distribution](figures/main/06_usopen_speed_distribution.png)
...
$$
\mathbf v_{\mathrm{rel}} = \mathbf v_{\mathrm{ball}} - \mathbf v_{\mathrm{air}}.
$$
